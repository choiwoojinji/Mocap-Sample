...
**현재 상태:** 데이터 수집 대기. 완료: `src/capture/landmark_viewer.py` (랜드마크 시각화), `src/dataset/collect.py` (시퀀스 수집), `src/training/train.py` (LSTM 학습·평가·저장 — 합성 데이터로 검증됨). 다음: 수신호 데이터 수집 → 실제 학습 → 실시간 추론(`src/inference/`). 상세 설계는 `doc/design.md`, ML 학습·예측 플로우는 `doc/ml-flow.md` 참고.
...
- `src/training/` — 모델 학습 (LSTM/GRU 계열 시퀀스 분류)
...
- **설계 결정** (`doc/design.md` 참고): 동적 수신호 30프레임 시퀀스, 특징은 양손+상체 포즈 150차원(`src/capture/extractor.py` — 수집·변환·추론이 모두 이 모듈을 사용해야 형식이 일치한다), 데이터는 `asset/<라벨>/*.npy` shape (30, 150). **클래스 목록은 하드코딩 금지** — `asset/`의 라벨 폴더에서 동적으로 구성하고 라벨 목록은 모델과 함께 저장·로드한다. 신뢰도가 낮으면 '정지'가 안전 기본값. 미정: 분류 결과 전달 방식 — 이게 중요한 작업에서는 임의로 가정하지 말고 사용자에게 확인할 것.
